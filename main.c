 ...
 * @details     Research-grade hearing assessment with frequency-specific analysis - CONTINUOUS OPERATION
 * @note        For MAX78000 EV Kit - Runs indefinitely with 3-minute hearing score updates
 */
...
// CONTINUOUS OPERATION - 3 MINUTE CYCLES
#define CYCLE_DURATION_SEC 180     // 3 minutes per hearing score update cycle
...
#define MAX_DOSE_WINDOWS (CYCLE_DURATION_SEC / DOSE_WINDOW_SIZE_SEC)   // 45 dose windows in 3 minutes
...
float total_dose_exposure = 0.0f;      // Running total of dose exposure for current cycle
float hearing_health_score = PERFECT_SCORE; // Current hearing health score (100=perfect, 0=worst)
uint32_t dose_window_index = 0;        // Current dose window number in cycle
float dose_history[MAX_DOSE_WINDOWS];  // History of dose values for each window
uint32_t cycle_start_time = 0;         // Cycle start time
uint32_t current_cycle = 0;            // Current cycle number (increments every 3 minutes)
...
float cumulative_health_score = PERFECT_SCORE;  // Overall health score across all cycles
float cycle_peak_dose = 0.0f;          // Peak dose in current cycle
float cycle_peak_critical = 0.0f;      // Peak critical frequency in current cycle
uint32_t total_runtime_sec = 0;        // Total runtime in seconds
...
void process_dose_window(void)
{
    ...
    // Track cycle peaks
    if (window_dose > cycle_peak_dose) {
        cycle_peak_dose = window_dose;
    }
    ...
    // Track critical frequency peaks
    if (critical_freq_exposure > cycle_peak_critical) {
        cycle_peak_critical = critical_freq_exposure;
    }
    ...
    // Calculate elapsed time for current cycle
    uint32_t elapsed_sec = (dose_window_index + 1) * DOSE_WINDOW_SIZE_SEC;
    total_runtime_sec += DOSE_WINDOW_SIZE_SEC;  // Track total runtime
    printf("Cycle time: %d/%d sec\n", elapsed_sec, CYCLE_DURATION_SEC);
    printf("Total runtime: %d sec (%.1f hours)\n", total_runtime_sec, total_runtime_sec / 3600.0f);
    printf("Cycle #: %d\n", current_cycle + 1);
    printf("-------------------\n");
    ...
    // Check if 3-minute cycle is complete
    if (dose_window_index >= MAX_DOSE_WINDOWS) {
        printf("=== 3-MINUTE CYCLE %d COMPLETE ===\n", current_cycle + 1);
        printf("Cycle Duration: %d seconds\n", CYCLE_DURATION_SEC);
        printf("Cycle Exposure: %.3f%%\n", total_dose_exposure);
        printf("Peak Window Dose: %.3f%%\n", cycle_peak_dose);
        printf("Peak Critical Freq: %.1f dB\n", cycle_peak_critical);
        printf("HEARING HEALTH SCORE: %.1f/100\n", hearing_health_score);
        
        // Update cumulative health score (weighted average)
        if (current_cycle == 0) {
            cumulative_health_score = hearing_health_score;
        } else {
            // Weighted average: give more weight to recent cycles
            cumulative_health_score = (cumulative_health_score * 0.7f) + (hearing_health_score * 0.3f);
        }
        
        // Final frequency-specific assessment for this cycle
        if (critical_freq_exposure > 85.0f) {
        ...
        // Overall cycle result with validation
        if (isnan(hearing_health_score)) {
            printf("CYCLE RESULT: ERROR - Invalid calculation\n");
        } else if (hearing_health_score >= 90.0f) {
            printf("CYCLE RESULT: EXCELLENT\n");
            printf("All frequencies protected!\n");
        } else if (hearing_health_score >= 75.0f) {
            printf("CYCLE RESULT: GOOD\n");
            printf("Good frequency balance\n");
        } else if (hearing_health_score >= 50.0f) {
            printf("CYCLE RESULT: FAIR\n");
            printf("Watch critical frequencies\n");
        } else if (hearing_health_score >= 25.0f) {
            printf("CYCLE RESULT: POOR\n");
            printf("Protect 3-6 kHz range!\n");
        } else {
            printf("CYCLE RESULT: CRITICAL\n");
            printf("DANGER: Critical freq damage!\n");
        }
        
        printf("Cumulative Health Score: %.1f/100\n", cumulative_health_score);
        printf("=== STARTING NEW 3-MINUTE CYCLE ===\n");
        
        // Reset for new 3-minute cycle
        current_cycle++;
        dose_window_index = 0;
        total_dose_exposure = 0.0f;
        hearing_health_score = PERFECT_SCORE;
        cycle_peak_dose = 0.0f;
        cycle_peak_critical = 0.0f;
        
        // Clear dose history for new cycle
        memset(dose_history, 0, sizeof(dose_history));
        
        printf("Cycle #%d started\n", current_cycle + 1);
        printf("Continuous monitoring active...\n");
        
        // Continue running - no test_active = 0 here!
    }
}
...
void perform_fft_based_analysis(void)
{
    ...
    // Debug audio input quality (less frequent for continuous operation)
    static uint32_t debug_counter = 0;
    debug_counter++;
    
    if (debug_counter % 16 == 0) {  // Debug every 16th analysis (reduced frequency)
        printf("Audio Debug:\n");
        printf("Non-zero samples: %d/%d\n", non_zero_count, FFT_SIZE);
        printf("Avg amplitude: %.1f\n", sample_sum / FFT_SIZE);
        printf("Cycle #%d, Window %d\n", current_cycle + 1, dose_window_index + 1);
    }
    ...
    if (non_zero_count > FFT_SIZE / 4) {  // At least 25% non-zero samples
        ...
        warning_counter++;
        if (warning_counter >= 40) {  // Reduce warning frequency
...
/**
 * @brief Initialize dose calculation system for continuous operation
 */
void init_dose_system(void)
{
    ...
    // Initialize continuous operation variables
    cumulative_health_score = PERFECT_SCORE;
    cycle_peak_dose = 0.0f;
    cycle_peak_critical = 0.0f;
    total_runtime_sec = 0;
    ...
    printf("=== CONTINUOUS HEARING MONITOR ===\n");
    printf("Research-grade frequency analysis\n");
    printf("RUNS INDEFINITELY - Turn off to stop\n");
    printf("Hearing score updates every 3 minutes\n");
    printf("Cycle Duration: %d seconds\n", CYCLE_DURATION_SEC);
...
int main(void)
{
    ...
    printf("CONTINUOUS OPERATION - 3-MINUTE HEARING SCORE UPDATES\n");
    ...
    printf("System will run until manually turned off\n");

    // CONTINUOUS OPERATION - RUNS FOREVER
    while (1) {
        /* Wait for I2S interrupt */
        while (i2s_flag == 0) {
            // Optional: Add periodic status updates here if needed
            static uint32_t status_counter = 0;
            status_counter++;
            
            // Print system status every ~60 seconds
            if (status_counter >= 600000) {  // Approximate timing
                printf("=== SYSTEM STATUS ===\n");
                printf("Runtime: %.1f hours\n", total_runtime_sec / 3600.0f);
                printf("Current Cycle: %d\n", current_cycle + 1);
                printf("Health Score: %.1f/100\n", hearing_health_score);
                printf("Cumulative Score: %.1f/100\n", cumulative_health_score);
                printf("==================\n");
                status_counter = 0;
            }
        }
    ...
    // This line will never be reached - system runs forever
    return 0;
}